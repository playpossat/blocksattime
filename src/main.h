...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.02 * COIN;	// 5% Interest 
...
public:
    ...
    {
        // satoshimadness: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // satoshimadness: get transaction coin age
...
public:
    ...
    // satoshimadness: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // satoshimadness: two types of block: proof-of-work or proof-of-stake
    ...
    // satoshimadness: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // satoshimadness: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // satoshimadness: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // satoshimadness: block index flags
    enum  
    {
